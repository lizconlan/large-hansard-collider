...
class Paragraph < ActiveRecord::Base
  ...
  def self.by_member(member_name)
    where(:type => "ContributionPara", :member => member_name)
  end
  
  def self.by_member_and_section_id(member_name, section_id)
    where(:type => "ContributionPara", :member => member_name, :section_id => section_id).sort(:sequence)
  end
  
  def self.by_member_and_section_id_start(member_name, section_start)
    where(:type => "ContributionPara", :member => member_name, :section_id => /^#{section_start}/).sort(:section_id, :sequence)
  end
end
